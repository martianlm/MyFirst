...
public class FastDFSClient {
	...
	static {
		try {
			String classPath = new File(FastDFSClient.class.getResource("/").getFile()).getCanonicalPath()
					+ File.separator + "fdfs_client.conf";
	...
	/*
	 * public FastDFSClient(String conf) throws Exception { if
	 * (conf.contains("classpath:")) { String path =
	 * URLDecoder.decode(getClass().getProtectionDomain().getCodeSource().
	 * getLocation().toString(), "UTF-8"); path = path.substring(6); conf =
	 * conf.replace("classpath:", URLDecoder.decode(path, "UTF-8")); }
	 * ClientGlobal.init(conf); trackerClient = new TrackerClient();
	 * trackerServer = trackerClient.getConnection(); storageServer = null;
	 * storageClient = new StorageClient1(trackerServer, storageServer); }
	 */
	 ...
	 *            M00/00/00/wKgRsVjtwpSAXGwkAAAweEAzRjw471.jpg
	 * @return
	 */
